  ...
  run: async (client, button) => {
                ...
                permissionOverwrites: [
                  {
                    ...
                    allow: ["SEND_MESSAGES", "914267469449953320"],
                  },
                  {
                    id: button.guild.roles.everyone,
                    deny: ["914267469449953320"],
                  },
                  {
    ...
    if (button.id === "claim_ticket") {
      ...
      con.query(
        `UPDATE Tickets SET claimed_staff = '${
          button.clicker.user.id
        }' WHERE pid = '${channelAuth('909782129909567532')}'`
      );
      button.channel.edit({
        topic: `**[DEVRALINDI: ${
          button.clicker.user.tag
    ...
    if (button.id === "reopen_ticket") {
      ...
      client.users.cache
        ...
        .send(
          `\`[SİSTEM]:\` Bir bilet tekrar açıldı. Kontrol etmek istersen <#${data[0][0].ticket_room}> 914267469449953320.`
        )
        .catch((e) => {
          button.channel.send({
    ...
    if (button.id === "archive_ticket") {
      ...
      let data = await con
        .promise()
        .query(
          `SELECT * FROM Tickets WHERE pid = '${channelAuth('914267469449953320')}'`
        );
      con.query(
